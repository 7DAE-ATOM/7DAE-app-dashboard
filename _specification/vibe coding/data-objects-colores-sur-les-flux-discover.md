...
- « Aléatoire » est retenu au sens « aucune sémantique, pas de palette métier à respecter » — pas au sens « différent à chaque fois ». La couleur est **dérivée de l'identité du data object** (son id technique), donc **identique d'un rendu à l'autre, d'une session à l'autre, et entre l'arbre et les flèches**.
- Un tirage réel serait redistribué à chaque ajout de nœud sur le canevas : la légende changerait sous les yeux de l'utilisateur au moindre dépliage de voisinage, et deux captures du même diagramme ne seraient pas comparables.
